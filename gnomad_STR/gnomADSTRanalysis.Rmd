...
# has extra columns for conditions and allele counts
# AR_total_sex <- subset(total, Inheritance == "AR")
# AR_Allele21_greater_than_path_min <- (AR_total_sex$Allele2LowerBound >= AR_total_sex$pathogenic_min) & (AR_total_sex$Allele1LowerBound >= AR_total_sex$pathogenic_min) 
# AR_motifs_match <- (AR_total_sex$motif_norm == AR_total_sex$repeatunit_path_normalized) 
# AR_result1 <- aggregate(cbind(AR_Allele21_greater_than_path_min, AR_motifs_match), by = list(AR_total_sex$Id), FUN = sum, na.rm=TRUE)
# AR_result2 <- aggregate(cbind(AR_total_sex$Allele2), by = list(AR_total_sex$Id, AR_total_sex$repeatunit_path_normalized), FUN = function(x) sum(!is.na(x)))
# AR_result <- merge(AR_result1, AR_result2)
...
# has extra columns for conditions and allele counts
# XR_XX_total_sex <- subset(total, Inheritance == "XR" & Sex == "XX")
# XR_XX_Allele1_greater_than_path_min <- (XR_XX_total_sex$Allele1LowerBound >= XR_XX_total_sex$pathogenic_min) & (XR_XX_total_sex$motif_norm == XR_XX_total_sex$repeatunit_path_normalized)
# XR_XX_Allele2_greater_than_path_min <- (XR_XX_total_sex$Allele2LowerBound >= XR_XX_total_sex$pathogenic_min) & (XR_XX_total_sex$motif_norm == XR_XX_total_sex$repeatunit_path_normalized)
# XR_XX_result1 <- aggregate(cbind(XR_XX_Allele1_greater_than_path_min, XR_XX_Allele2_greater_than_path_min), by = list(XR_XX_total_sex$Id), FUN = sum, na.rm=TRUE)
# XR_XX_result2 <- aggregate(cbind(XR_XX_total_sex$Allele1, XR_XX_total_sex$Allele2), by = list(XR_XX_total_sex$Id, XR_XX_total_sex$repeatunit_path_normalized), FUN = function(x) sum(!is.na(x)))
# XR_XX_result <- merge(XR_XX_result1, XR_XX_result2)
...
# XR_XY_result2 <- aggregate(cbind(XR_XY_total_sex$Allele1, XR_XY_total_sex$Allele2), by = list(XR_XY_total_sex$Id, XR_XY_total_sex$repeatunit_path_normalized), FUN = function(x) sum(!is.na(x)))
