...
result <- calculate_ci(result, successes_col = "Allele2>pathogenic_max&agemax>age_onset_max&motif_norm=motif_path", trials_col = "Allele2_Loci_by_Id", conf_level = 0.95)
...
ggplot(subset(result, percent > 5), aes(x = reorder(Id, percent), y = percent, color = pathogenic_motif)) +
  ...
  geom_errorbar(aes(ymin = lower_ci, ymax = upper_ci), width = 0.2) +
  xlab("Id") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size =8)) +
  ylab("Pathogenic Percentage") + ggtitle("Pathogenic Percentage Over Age by Id AND Motif with 95% CI (Percent > 5)")
