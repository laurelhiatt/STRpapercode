...
for (locus in unique(gnomADSTRcalls$Id)) {
  ...
  p <- tidy(wilcox.test(subset(gnomADSTRcalls, PcrProtocol == 'pcr_free')$Allele2, 
             subset(gnomADSTRcalls, PcrProtocol == 'pcr_plus')$Allele2))
  cat('\n') 
  cat("The statistic is ", cat(p$statistic))
  cat('\n') 
  cat("The p-value is ",cat(p$p.value))
   cat('\n') 
}
```
...
Allele2_greater_than_path_max <- total$Allele2 >= total$pathogenic_max
...
result$percent<- (result$`Count Allele2 > pathogenic_max`/result$`Total L Allele2Loci for Id`)*100
